...
Tree-based ensembled models including Random forest and Generalized boosted regression are built. We noticed that Categorical variable `team` have 30 classes, which may result in too much dummy variables and cause overfitting of tree-based models. Therefore, we consider clustering `team` into 3 classes according to similar trends in the median and standard deviation of player's salary in each team. We use the new variable `team_cluster` instead of `team` in the following random forest and gbm models.
...
figure_tree_1  = km_vis + theme_bw() + xlab("Median") + ylab("SD") + 
  theme(plot.title = element_blank()) 
ggsave("report_figures/figure_tree_1.png",figure_tree_1,dpi = 300)
