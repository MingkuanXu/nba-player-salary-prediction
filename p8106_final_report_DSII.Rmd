...
The elastic-net model has two parameters, which are $\alpha$ (compromise between LASSO and ridge) and $\lambda$ (the penalty term limits the number or magnitude of predictor coefficients). The elastic-net model reached its best tune at $\alpha = 0.6$ and $\lambda = 0.44$ (see Appendix B Figure 9).

#### (3) Principle Component Regression

The tuning parameter of PCR is the number of principal components (PCs) included in the final model. There are 12 PCs included in the model with minimum RMSE (see Appendix B Figure 10). 
